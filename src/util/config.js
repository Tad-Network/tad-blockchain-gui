...
global.daemon_rpc_ws = `wss://${self_hostname}:4044`;
...
function loadConfig(net) {
  try {
    // check if TAD_ROOT is set. it overrides 'net'
    const config_root_dir =
      'TAD_ROOT' in process.env
        ? process.env.TAD_ROOT
        : path.join(os.homedir(), '.tad', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 4044); // jshint ignore:line
