...
export default function Block() {
  ...
  const poolReward = mtad_to_tad(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mtad_to_tad(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const tadFees = blockRecord.fees
    ? mtad_to_tad(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tadexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tadexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
