...
import { /* mtad_to_tad_string, */ tad_to_mtad } from '../../../util/tad';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mtad_to_tad_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of tad in the blockchain at the current
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send tad to coloured address. Please enter a tad
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    const amountValue = Number.parseFloat(tad_to_mtad(amount));
    const feeValue = Number.parseFloat(tad_to_mtad(fee));
