...
import { AlertDialog, Card, Flex } from '@tad/core';
...
import {
  mtad_to_colouredcoin_string,
  colouredcoin_to_mtad,
} from '../../../util/tad';
      ...
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mtad_to_colouredcoin_string(props.balance)} {cc_unit}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_tad = mtad_to_colouredcoin_string(balance_ptotal);
  const balance_pending_tad = mtad_to_colouredcoin_string(balance_pending);
  const balance_change_tad = mtad_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = colouredcoin_to_mtad(amount_input.value);
    const fee = colouredcoin_to_mtad(fee_input.value);
